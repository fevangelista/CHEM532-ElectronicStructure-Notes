...
By combining spin functions with spatial orbitals we get \textbf{spin orbitals}. The most general form of a spin orbital is\mnote{We will abbreviate the spatial and spin coordinates with the symbol $x \equiv (\vec{r},\omega)$.}
\begin{equation}
...
= &
\int d\vec{r} \, \phi_{i}^{\sigma}(\vec{r})^* \phi_{j}^{\sigma'}(\vec{r})
\int d\omega \, 
