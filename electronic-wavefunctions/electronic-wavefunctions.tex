...
\Phi(x_1,x_2) = \sum_{i}^{\infty} a_i(x_2) \phi_i(x_1),
\end{equation}
...
a_i(x_2) = \sum_{j}^{\infty} a_{ij} \phi_j(x_2),
\end{equation}
...
\begin{equation}
\Phi(x_1,x_2) = \sum_{ij}^{\infty} a_{ij} \phi_i(x_1) \phi_j(x_2).
\end{equation}
This functions may satisfy the antisymmetry condition if:
\begin{equation}
\sum_{ij}^{\infty} a_{ij} \phi_i(x_1) \phi_j(x_2) = -\sum_{ij}^{\infty} a_{ij} \phi_i(x_2) \phi_j(x_1).
\end{equation}
...
\begin{equation}
\sum_{ij}^{\infty} a_{ij} \phi_i(x_1) \phi_j(x_2) = -\sum_{ij}^{\infty} a_{ji} \phi_j(x_2) \phi_i(x_1),
\end{equation}
and the collect terms that multiply the factor $\phi_i(x_1) \phi_j(x_2)$ to obtain the condition:
\begin{equation}
\label{eq:two_electron_wfn_expansion4}
\sum_{ij}^{\infty} (a_{ij} + a_{ji}) \phi_i(x_1) \phi_j(x_2) = 0.
\end{equation}
Equation~\eqref{eq:two_electron_wfn_expansion4} must hold for any value of $\phi_i(x_1) \phi_j(x_2)$, and this is possible only if
...
\begin{equation}
\begin{split}
\Phi(x_1,x_2) &= \sum_{i<j}^{\infty} a_{ij} \phi_i(x_1) \phi_j(x_2) + \sum_{i>j}^{\infty} a_{ij} \phi_i(x_1) \phi_j(x_2) \\
&= \sum_{i<j}^{\infty} a_{ij} \phi_i(x_1) \phi_j(x_2) + \sum_{j>i}^{\infty} a_{ji} \phi_j(x_1) \phi_i(x_2) \\
&= \sum_{i<j}^{\infty} a_{ij} \phi_i(x_1) \phi_j(x_2) - \sum_{i<j}^{\infty} a_{ij} \phi_i(x_2) \phi_j(x_1) \\
&= \sum_{i<j}^{\infty} a_{ij} [ \phi_i(x_1) \phi_j(x_2) - \phi_i(x_2) \phi_j(x_1)].
\end{split}
\end{equation}
The quantity $\phi_i(x_1) \phi_j(x_2) - \phi_i(x_2) \phi_j(x_1)$ can also be expressed as the determinant of a matrix since:
\begin{equation}
\det
\begin{bmatrix}
\phi_i(x_1) & \phi_i(x_2) \\
...
\end{vmatrix}
=
\phi_i(x_1) \phi_j(x_2) - \phi_i(x_2) \phi_j(x_1).
\end{equation}
...
\begin{vmatrix}
\phi_i(x_1) & \phi_i(x_2) & \cdots & \phi_i(x_N) \\
\phi_j(x_1) & \phi_j(x_2) & \cdots & \phi_j(x_N) \\
\phi_k(x_1) & \phi_k(x_2) & \cdots & \phi_k(x_N) \\
...
\phi_i \phi_j \phi_k \cdots},
\end{equation}
omitting the labels of the electrons.
The basis of Slater determinants $\{ \ket{
\phi_i \phi_j \phi_k \cdots} \}$ with $i, j, k, \ldots \in \{0, 1, 2, \ldots \}$ is complete.\mnote{This property is inherited from the fact that the one-electron spin orbital basis is complete.}
...
\end{equation}
If instead the two electrons have different spin, then the cross terms [$\phi_i^*(\mathbf{r}_1)\phi_j(\mathbf{r}_1)  \phi_j^*(\mathbf{r}_2) \phi_i(\mathbf{r}_2)$] survive and introduce correlation in the probability distribution
\begin{equation}
\begin{split}
P(\mathbf{r}_1,\mathbf{r}_2) = \frac{1}{2} [&
P_i(\mathbf{r}_1) P_j(\mathbf{r}_2) +  P_j(\mathbf{r}_1) P_i(\mathbf{r}_2) \\
...
\phi_{\mathrm{u}}(\mathbf{r}) &= N_\mathrm{g} [\chi_{1\rm s}^{\rm A}(\mathbf{r}) - \chi_{1\rm s}^{\rm B}(\mathbf{r})].
\end{align}
...
\end{split}
\end{equation}
Where in the last step we used the antisymmetry property of Slater determinants.\mnote{$\ket{\chi_{1\rm s}^{\rm A}\alpha \chi_{1\rm s}^{\rm B}\alpha} = -\ket{\chi_{1\rm s}^{\rm B}\alpha \chi_{1\rm s}^{\rm A}\alpha}$.}
...
\begin{equation}
\hat{S}^2 \ket{\Phi_\pm} = (1 \pm 1) \ket{\Phi_\pm}.
\end{equation}
